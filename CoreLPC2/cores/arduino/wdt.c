...
void wdt_init(uint32_t s_counter){
    ...
    LPC_WDT->WDCLKSEL = 0x1;                // Set CLK src to PCLK (PCLK_WDT is default to CCLK/4)
    uint32_t clk = SystemCoreClock / 16;    // WDT has a fixed /4 prescaler, PCLK default is /4
    LPC_WDT->WDTC = s_counter * clk;
    ...
    LPC_WDT->WDMOD = (1<<WDEN_SBIT);// | (1<<WDRESET_SBIT); //Enable Watchdog and Enable Watchdog to reset CPU on timeout
